...
describe('转化型技能', () => {
  ...
  it('转换只覆盖【打击】与【防御】：其他牌没有转化用法，也不能作为响应打出', () => {
    const state = makeState({
      playerSpecies: 'morph',
      aiSpecies: 'defensive',
      playerHand: [{ kind: 'storm' }],
    })
    const storm = state.players[0].hand[0]!

    expect(useOptions(state, 0, storm).map((o) => o.as)).toEqual(['storm'])
    expect(playOptions(state, 0, storm)).toHaveLength(0)
  })
...
describe('转化由文档描述', () => {
  it('换一份技能文档即可新增一条转化，引擎与 legality 都不用改', () => {
    ...
    const defend = state.players[0].hand[0]!

    // 防御型的蓄能本来只是常驻修正：只有"使用【防御】"这一种牌面
    expect(useOptions(state, 0, defend).map((o) => o.as)).toEqual(['defend'])

    withRegistry(synthetic, () => {
      expect(useOptions(state, 0, defend)).toEqual([
        { as: 'defend' },
        { as: 'strike', via: 'charge' },
      ])
      // 端到端：把【防御】当【打击】使用，对手获得 1 点威胁
      submit(state, { kind: 'use-card', card: defend, as: 'strike', via: 'charge' })
      expect(state.players[1].threat).toBe(1)
