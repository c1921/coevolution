...
describe('卡牌目标选择入口', () => {
  ...
  it('回复：候选是自己（满血与否由变体的 requires 把关，候选本身不变）', () => {
    const wounded = makeState({ playerSpecies: 'tiger', aiSpecies: 'bear', playerHp: 2 })
    ...
    // 目标解析本身仍给出唯一候选
    const healthy = makeState({ playerSpecies: 'tiger', aiSpecies: 'bear' })
  ...
  it('急救：候选随受伤情况变化，双候选时要求选择', () => {
    ...
    const onlyOpponent = makeState({ playerSpecies: 'tiger', aiSpecies: 'bear', aiHp: 2 })
  ...
  it('濒死语境用 dying 变体的目标规格，并绑定濒死者', () => {
    const state = makeState({ playerSpecies: 'tiger', aiSpecies: 'bear', playerHp: 0 })
