 ...
 *
 * 内置内容已删除全部回血牌，因此「自己回复」「任意已受伤角色」两类目标规格
 * 由 `fixtures.ts` 的合成牌（`test-mend` / `test-aid`）提供；它们只在测试注册表里。
 */
function withHeal<T>(fn: () => T): T {
...
describe('卡牌目标选择入口', () => {
  ...
  it('回复：候选是自己（满血与否由变体的 requires 把关，候选本身不变）', () => {
    withHeal(() => {
      const wounded = makeState({
        playerSpecies: 'offensive',
        aiSpecies: 'defensive',
        playerHp: 2,
      })
      expect(cardTargetChoice(wounded, 0, 'test-mend', 'play')).toMatchObject({
        candidates: [0],
        fallback: 0,
        mustChoose: false,
      })
      ...
      expect(cardTargetChoice(healthy, 0, 'test-mend', 'play')).toMatchObject({ candidates: [0] })
    })
  })
  ...
  it('急救：候选随受伤情况变化，双候选时要求选择', () => {
    withHeal(() => {
      ...
      expect(cardTargetChoice(bothWounded, 0, 'test-aid', 'play')).toMatchObject({
        candidates: [0, 1],
        fallback: 0,
        mustChoose: true,
        multi: false,
      ...
      expect(cardTargetChoice(onlyOpponent, 0, 'test-aid', 'play')).toMatchObject({
        candidates: [1],
        mustChoose: true,
      })
      expect(cardTargetChoice(onlyOpponent, 0, 'test-aid', 'play')?.fallback).toBeUndefined()

      // 谁都没受伤 → 一个候选都没有
      const healthy = makeState({ playerSpecies: 'offensive', aiSpecies: 'defensive' })
      expect(cardTargetChoice(healthy, 0, 'test-aid', 'play')).toBeNull()
    })
  })
  ...
  it('濒死语境用 dying 变体的目标规格，并绑定濒死者', () => {
    withHeal(() => {
      ...
      expect(cardTargetChoice(state, 0, 'test-mend', 'dying')).toBeNull()
      expect(cardTargetChoice(state, 0, 'test-mend', 'dying', 0)).toMatchObject({
        candidates: [0],
        mustChoose: true,
      })
    })
...
describe('缺省目标集与可满足性', () => {
  ...
  it('单选且必须选择时用缺省目标，缺省不合格时退回第一个候选', () => {
    withHeal(() => {
      ...
      const choice = cardTargetChoice(onlyOpponent, 0, 'test-aid', 'play')
