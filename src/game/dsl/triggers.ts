...
import type { DamageCtx, GameState, PlayerIndex, ThreatCtx, TriggerRef } from '../types'
 ...
 *  - 技能触发要区分可选/不可选：不可选的立即执行，可选的在 after-damage /
 *    after-threat 上交给对应的结算帧逐个询问——这样"可选询问"与主结算的顺序由引擎掌控。
 */

/** 事件负载：伤害事件与威胁事件（将来扩展新时机时在这里加字段） */
...
export function contextFor(
  ...
  return {
    ...
    target: payload?.damage?.target ?? payload?.threat?.target,
    source: payload?.damage?.source ?? payload?.threat?.source,
