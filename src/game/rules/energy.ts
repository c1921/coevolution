 ...
 * 濒死时使用【回复】都要支付能量。费用按「**当作的牌面**」计算，所以转化牌
 * （猛扑 / 疾影 / 灵草）付的是转化后那张牌的费用，例如红牌当【回复】要付 2 点。
 *
 * 不消耗能量的行为：主动技（疗愈 / 透支）、弃牌阶段的弃置、疗愈的弃牌——
 * 它们都不是「使用 / 打出」一张牌。
 *
 * 重要不变量：**任何牌面的费用都必须 ≥ 1**。费用一旦可以为 0，
...
export function canPayEnergy(state: GameState, p: PlayerIndex, as: CardKind): boolean {
  return state.players[p].energy >= energyCost(as)
}
...
export function shortfallReason(
  ...
  return `能量不足：使用【${CARD_DEFS[as].name}】需要 ${energyCost(as)} 点能量（当前 ${state.players[p].energy} 点）`
}
