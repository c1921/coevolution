...
describe('能量系统', () => {
  it('费用按牌种固定：打击 1 / 防御 1 / 风暴 2，且与牌面说明一致', () => {
    ...
    expect(energyCost(state, 0, 'storm')).toBe(2)

    // 全部内置牌种（含奖励新卡与升级版）的费用都要与牌面文案一致
    for (const kind of cardIds()) {
  ...
  it('能量为 0 时没有任何可用的牌面', () => {
    const state = makeState({
      ...
      aiSpecies: 'defensive',
      playerEnergy: 0,
      playerHand: [{ kind: 'strike' }, { kind: 'defend' }, { kind: 'storm' }],
    })
  ...
  it('payEnergy 在能量不足时抛错且不扣减', () => {
    ...
    expect(() => payEnergy(state, 0, 'storm')).toThrow('能量不足')
